...
namespace aur {
    ...
    std::pair<std::vector<unsigned int>, std::vector<Vertex>> generate_triangle_geometry_data(float size = 1.0f, std::vector<glm::vec4> colors = std::vector<glm::vec4>{ {1.0f, 1.0f, 1.0f, 1.0f } }) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                vertices.push_back(Vertex{{x, y, 0.0f}, colors[i] });
